...
/**
 * Função ler_aceleracao_y - lê a aceleração no eixo Y
 * @param y - ponteiro para a variável que armazenará a aceleração
 * @return void
 */
...
/**
 * Função calibrar_acelerometro - calibra o offset dos eixos X e Y
 * @param offset_x - ponteiro para a variável que armazenará o offset do eixo X
 * @param offset_y - ponteiro para a variável que armazenará o offset do eixo Y
 * @return void
 */
void calibrar_acelerometro(int16_t *offset_x, int16_t *offset_y) {
   ...
   *offset_y = soma_y / AMOSTRAS_CALIBRACAO;
   printf("Calibracao completa. Offset: X=%d, Y=%d\n", *offset_x, *offset_y);
}


/**
 * Função configura_acelerometro - inicializa o acelerômetro ADXL345
 * e calibra o offset dos eixos X e Y
 */
 ...
 * 0 - Sem movimento
 * 
 * Adicionalmente, se o movimento for detectado no eixo Y, a direção será baseada nesse eixo.
 */
int get_direcao_movimento(){
   ...
   // printf("X lido: %d, Y lido: %d, X lapidado: %.2f g, Y lapidado: %.2f g\n", x_bruto, y_bruto, x_g, y_g);
   ...
   if (x_bruto > 50) {
       ...
       //printf("Movimento no eixo Y (Para baixo)\n");
       return 2; // Novo valor para o movimento no eixo Y
   } else if (y_bruto < -50) {
       //printf("Movimento no eixo Y (Para cima)\n");
       return -2; // Novo valor para o movimento no eixo Y
   } else {
